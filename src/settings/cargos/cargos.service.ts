...
export class CargosService {
    ...
    async create(createCargoDto: CreateCargoDto) {
        try {
            ...
            if (error.code === '23505') {
                throw new ConflictException(`El cargo con código ${createCargoDto.codigo} ya existe`);
            }
            throw error;
        }
    }
    ...
    async update(id: string, updateCargoDto: UpdateCargoDto) {
        ...
        try {
            ...
            if (error.code === '23505') {
                throw new ConflictException(`El cargo con código ${updateCargoDto.codigo} ya existe`);
            }
            throw error;
        }
    }
