...
export class CentrosCostosService {
  ...
  async create(createCentroCostoDto: CreateCentroCostoDto) {
    try {
      const centroCosto =
        this.centrosCostosRepository.create(createCentroCostoDto);
      return await this.centrosCostosRepository.save(centroCosto);
    } catch (error) {
      if (error.code === '23505') {
        throw new ConflictException(
          `El centro de costo con código ${createCentroCostoDto.codigo} ya existe`,
        );
      }
      throw error;
    }
  ...
  async findOne(id: string) {
    const centroCosto = await this.centrosCostosRepository.findOne({
      where: { id },
    });
    if (!centroCosto) {
  ...
  async update(id: string, updateCentroCostoDto: UpdateCentroCostoDto) {
    ...
    try {
      ...
      if (error.code === '23505') {
        throw new ConflictException(
          `El centro de costo con código ${updateCentroCostoDto.codigo} ya existe`,
        );
      }
      throw error;
    }
