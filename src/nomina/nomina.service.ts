...
export class NominaService {
    ...
    async liquidarPeriodo(periodoId: string, dto: LiquidarNominaDto, userId: string) {
        ...
        const empleadosItems = dto.empleados?.length
            ? dto.empleados
            : (await this.empleadoRepo.find({ where: { activo: true } }))
                .map(e => ({ empleadoId: e.id, diasTrabajados: 30 }));
        ...
        const totalCosto = saved.reduce((s, l) => s + Number(l.totalDevengado) + Number(l.totalAportes) + Number(l.totalProvisiones), 0);
    ...
    async pagarNomina(periodoId: string, dto: PagarNominaDto, userId: string) {
        ...
        if (periodo.estado !== EstadoPeriodoNomina.LIQUIDADA) {
            throw new BadRequestException('El período debe estar en estado LIQUIDADA para pagarlo');
        }

        const asiento = await this.asientosContablesService.generarAsientoPagoNomina({
            periodoNombre: periodo.nombre,
    ...
    async anularNomina(periodoId: string, userId: string) {
        const periodo = await this.findOnePeriodo(periodoId);
        if (periodo.estado === EstadoPeriodoNomina.BORRADOR) {
            throw new BadRequestException('No se puede anular un período en estado BORRADOR');
        }
        ...
        if (periodo.asientoPagoId) {
            const asientoPago = await this.asientosContablesService.findOneAsientoConDetalles(periodo.asientoPagoId);
    ...
    async findAllEntidadesSS(tipo?: string) {
        const where = tipo ? { tipo: tipo as TipoEntidadSS, activo: true } : { activo: true };
        return this.entidadSSRepo.find({ where, order: { nombre: 'ASC' } });
    }

    async findAllCargos() {
        return this.cargoRepo.find({ where: { activo: true }, order: { nombre: 'ASC' } });
    }

    async findAllCentrosCosto() {
        return this.centroCostoRepo.find({ where: { activo: true }, order: { nombre: 'ASC' } });
    }
    ...
    private async calcularLiquidacion(
        ...
        const auxilioTransporte = (empleado.auxilioTransporte && Number(empleado.salarioBase) <= 2 * SMMLV_2026)
            ? Math.round((AUXILIO_TRANSPORTE_2026 / 30) * dias * 100) / 100
            : 0;

        const totalHorasExtras = (item.horasExtras || []).reduce((s, h) => s + Number(h.valor), 0);
        const totalBonificaciones = (item.bonificaciones || []).reduce((s, b) => s + Number(b.valor), 0);
        const comisiones = item.comisiones || 0;

        const totalDevengado = Math.round((
            salarioDevengado + auxilioTransporte + totalHorasExtras + totalBonificaciones + Number(comisiones)
        ) * 100) / 100;
        ...
        const totalOtrasDeducciones = otrasDeducciones.reduce((s, d) => s + Number(d.valor), 0);

        const totalDeducciones = Math.round((
            saludEmpleado + pensionEmpleado + retencionFuente + totalOtrasDeducciones
        ) * 100) / 100;

        const netoPagar = Math.round((totalDevengado - totalDeducciones) * 100) / 100;

        // ── Aportes empleador ──
        const tasasARL = [0, 0.00348, 0.01044, 0.02436, 0.04350, 0.08700];
        ...
        const aportes = [
            ...
            ...(aplicaSENA ? [{ concepto: 'SENA', valor: Math.round(ibc * 0.02 * 100) / 100 }] : []),
            ...(aplicaICBF ? [{ concepto: 'ICBF', valor: Math.round(ibc * 0.03 * 100) / 100 }] : []),
        ];
    ...
    private calcularRetencionFuente(salarioBase: number, totalDevengado: number, salarioIntegral: boolean): number {
        if (salarioIntegral) {
    ...
    async reporteCostosPorCentroCosto(fechaInicio?: string, fechaFin?: string) {
        const qb = this.liquidacionRepo.createQueryBuilder('l')
        ...
        for (const l of liquidaciones) {
            ...
            g.totalCosto += Number(l.totalDevengado) + Number(l.totalAportes) + Number(l.totalProvisiones);
        }
    ...
    async reporteComparativoPeriodos(periodo1Id: string, periodo2Id: string) {
        ...
        const [liq1, liq2] = await Promise.all([
            this.liquidacionRepo.find({ where: { periodoId: periodo1Id }, relations: ['empleado'] }),
            this.liquidacionRepo.find({ where: { periodoId: periodo2Id }, relations: ['empleado'] }),
        ]);

        const sumar = (items: any[]) => ({
            empleados: items.length,
            totalDevengado: Math.round(items.reduce((s, l) => s + Number(l.totalDevengado), 0) * 100) / 100,
            totalDeducciones: Math.round(items.reduce((s, l) => s + Number(l.totalDeducciones), 0) * 100) / 100,
            totalNeto: Math.round(items.reduce((s, l) => s + Number(l.netoPagar), 0) * 100) / 100,
            totalAportes: Math.round(items.reduce((s, l) => s + Number(l.totalAportes), 0) * 100) / 100,
            totalProvisiones: Math.round(items.reduce((s, l) => s + Number(l.totalProvisiones), 0) * 100) / 100,
            saludEmpleado: Math.round(items.reduce((s, l) => s + Number(l.saludEmpleado), 0) * 100) / 100,
            pensionEmpleado: Math.round(items.reduce((s, l) => s + Number(l.pensionEmpleado), 0) * 100) / 100,
            retencionFuente: Math.round(items.reduce((s, l) => s + Number(l.retencionFuente), 0) * 100) / 100,
            costoEmpresa: Math.round(items.reduce((s, l) => s + Number(l.totalDevengado) + Number(l.totalAportes) + Number(l.totalProvisiones), 0) * 100) / 100,
        });

        const detalleEmpleados = liq1.map(l1 => {
            const l2 = liq2.find(l => l.empleadoId === l1.empleadoId);
            const nombre = `${l1.empleado?.primerNombre || ''} ${l1.empleado?.primerApellido || ''}`.trim();
            return {
                ...
                variacion: l2 ? Math.round((Number(l2.totalDevengado) - Number(l1.totalDevengado)) * 100 / (Number(l1.totalDevengado) || 1) * 100) / 100 : null,
            };
        });
    ...
    async reporteResumenAportes(periodoId: string) {
        ...
        const aportesMap = new Map<string, { concepto: string; valor: number; empleados: Set<string> }>();

        for (const l of liquidaciones) {
            const aportes: { concepto: string; valor: number }[] = l.aportesEmpleador as any || [];
            for (const ap of aportes) {
                if (!aportesMap.has(ap.concepto)) {
                    aportesMap.set(ap.concepto, { concepto: ap.concepto, valor: 0, empleados: new Set() });
                }
        ...
        const totales = {
            totalAportes: Math.round(data.reduce((s, d) => s + d.valor, 0) * 100) / 100,
            totalEmpleados: liquidaciones.length,
            totalDevengado: Math.round(liquidaciones.reduce((s, l) => s + Number(l.totalDevengado), 0) * 100) / 100,
        };

        // Aportes por empleado
        const detalleEmpleados = liquidaciones.map(l => {
            const nombre = `${l.empleado?.primerNombre || ''} ${l.empleado?.primerApellido || ''}`.trim();
            return {
